...
var features = test1_feature; //Change here
...
function preload() {
  song = loadSound("../music_library/test1.mp3"); //Change here
}

function setup() {
  var canvas= createCanvas(600, 600);
  canvas.parent("song_1")
  angleMode(DEGREES);
  imageMode(CENTER);
...
function draw() {
  ...
  fft.analyze();
  amp_beat = fft.getEnergy(5, 200);
  amp_2 = fft.getEnergy(5, 100);
  amp = fft.getEnergy(101, 300); //Get the frequency betweeen 20~200 that are low frequencies
  amp_3 = fft.getEnergy(301, 500);
  amp_4 = fft.getEnergy(501, 700);
  amp_5 = fft.getEnergy(701, 1000);
  amp_5 = fft.getEnergy(1200, 1300);
  //console.log(amp)
  var wave = fft.waveform();

  //Add condition to generate particles
  var par_gen_threshold = 60 * parseFloat(features.speechiness) * 10;
  var par_gen_random = getRandomInt(60);
  if (par_gen_threshold > par_gen_random) {
    var p = new Particle(features);
    particles.push(p);
  }

  for (var i = particles.length - 1; i >= 0; i--) {
    if (!particles[i].edges()) {
      particles[i].update(amp_beat > 220);
      particles[i].show();
    } else {
